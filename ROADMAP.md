...
| 工具面 | CLI 60 命令 · MCP 47 工具（含 inputSchema 字典）· WASM 双产物 37/38 导出（26 个 session API）· Node SDK |
...
**4. deepDesign Studio 全面接入新引擎。** 引擎能力面（47 工具/25 op/组件库/交互面板/主题令牌）与 Studio 画布对齐：画布撤销/重做（引擎 revision + journal 已就绪）、组件库拖放、交互面板、源码视图直读 `.mbt.md`。
