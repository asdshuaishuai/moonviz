...
> 当前基线：**v0.1.6-fix** · `moon test` 209/209 · 组件目录 65×115 · 仓库 100+ commits
...
| 工具面 | CLI 65 命令 · MCP 53 工具（含 inputSchema 字典）· WASM 双产物 72/53 导出（session 28）· Node SDK |
...
**4. deepDesign Studio 全面接入新引擎。** 引擎能力面（53 工具/26 op/组件库/交互面板/主题令牌）与 Studio 画布对齐：画布撤销/重做（引擎 revision + journal 已就绪）、组件库拖放、交互面板、源码视图直读 `.mbt.md`。
